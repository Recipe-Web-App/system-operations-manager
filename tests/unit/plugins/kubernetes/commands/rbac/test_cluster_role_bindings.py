...
class TestClusterRoleBindingCommands:
    ...
    def test_list_cluster_role_bindings_kubernetes_error(
        self, cli_runner: CliRunner, app: typer.Typer, mock_rbac_manager: MagicMock
    ) -> None:
        """list should handle KubernetesError."""
        mock_rbac_manager.list_cluster_role_bindings.side_effect = KubernetesError(
            "connection failed"
        )
    ...
    def test_get_cluster_role_binding_kubernetes_error(
        self, cli_runner: CliRunner, app: typer.Typer, mock_rbac_manager: MagicMock
    ) -> None:
        """get should handle KubernetesError."""
        mock_rbac_manager.get_cluster_role_binding.side_effect = KubernetesNotFoundError(
            resource_type="ClusterRoleBinding", resource_name="nonexistent"
        )

        result = cli_runner.invoke(app, ["cluster-role-bindings", "get", "nonexistent"])

        assert result.exit_code == 1
        assert "not found" in result.stdout.lower()

    def test_create_cluster_role_binding_kubernetes_error(
        self, cli_runner: CliRunner, app: typer.Typer, mock_rbac_manager: MagicMock
    ) -> None:
        """create should handle KubernetesError."""
        mock_rbac_manager.create_cluster_role_binding.side_effect = KubernetesError(
            "connection failed"
        )
        role_ref_json = (
            '{"kind":"ClusterRole","name":"cluster-admin","api_group":"rbac.authorization.k8s.io"}'
        ...
        result = cli_runner.invoke(
            app,
            ["cluster-role-bindings", "create", "test-crb", "--role-ref", role_ref_json],
        )

        assert result.exit_code == 1

    def test_delete_cluster_role_binding_aborts_without_confirmation(
        self, cli_runner: CliRunner, app: typer.Typer, mock_rbac_manager: MagicMock
    ) -> None:
        """delete without --force should abort when user declines."""
        result = cli_runner.invoke(
            app, ["cluster-role-bindings", "delete", "test-crb"], input="n\n"
        )

        assert result.exit_code != 0
        mock_rbac_manager.delete_cluster_role_binding.assert_not_called()

    def test_delete_cluster_role_binding_kubernetes_error(
        self, cli_runner: CliRunner, app: typer.Typer, mock_rbac_manager: MagicMock
    ) -> None:
        """delete should handle KubernetesError."""
        mock_rbac_manager.delete_cluster_role_binding.side_effect = KubernetesError(
            "connection failed"
        )
