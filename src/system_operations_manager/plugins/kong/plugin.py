...
class KongPlugin(Plugin):
    ...
    def register_commands(self, app: typer.Typer) -> None:
        ...
        Creates a 'kong' subcommand group with status and info commands.
    ...
    def _register_status_commands(self, app: typer.Typer) -> None:
        ...
        def status(
            ...
            try:
                ...
                if json_output:
                    ...
                    console.print(json.dumps({"status": status_data, "info": info_data}, indent=2))
                    return
                ...
                # Basic info
                table.add_row("Node", info_data.get("hostname", "unknown"))
                table.add_row("Version", info_data.get("version", "unknown"))
                ...
                table.add_row(
                    "Database", "Connected" if db_reachable else "[red]Disconnected[/red]"
                )

                if verbose:
                    # Add more details
                    table.add_row("Lua Version", info_data.get("lua_version", "unknown"))
                    table.add_row("Tagline", info_data.get("tagline", ""))
                    ...
                    if lua_shared := memory.get("lua_shared_dicts", {}):
                        ...
                        if allocated := workers.get("allocated_slabs"):
                            table.add_row("Memory (Kong)", allocated)
        ...
        def info(
            ...
            try:
                ...
                table.add_row(
                    "Admin API", plugin_config.connection.base_url if plugin_config else "N/A"
                )
                table.add_row("Version", info_data.get("version", "unknown"))
                table.add_row("Hostname", info_data.get("hostname", "unknown"))
                table.add_row("Lua Version", info_data.get("lua_version", "unknown"))
                ...
                table.add_row("Plugins Available", str(len(available)))
                table.add_row("Plugins Enabled", str(len(enabled)))
